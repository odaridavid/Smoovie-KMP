...
class GetMovieDetailUseCaseTest {
    ...
    fun `given providers have DE region - when invoked - then uses DE streaming data`() =
        runTest {
            ...
            val result = buildUseCase(repo).invoke(1)
    ...
    fun `given empty providers map - when invoked - then streaming and rentBuy are empty`() =
        runTest {
            val result = buildUseCase().invoke(1)
    ...
    fun `given providers fetch fails - when invoked - then streaming and rentBuy are empty without throwing`() =
        runTest {
            val delegate = FakeMoviesRepository(movieDetail = baseDetail)
            val repo =
                object : MoviesRepository by delegate {
            ...
            val repo =
                FakeMoviesRepository(
                    ...
                    keywords =
                        KeywordsResponse(
                            id = 1,
                            keywords = (1..5).map { Keyword(id = it, name = "keyword$it") },
                        ),
                )
    ...
    @Test
    fun `given exactly 3 keywords - when invoked - then all are returned`() =
        runTest {
            val repo =
                FakeMoviesRepository(
                    movieDetail = baseDetail,
                    keywords =
                        KeywordsResponse(
                            id = 1,
                            keywords = (1..3).map { Keyword(id = it, name = "kw$it") },
                        ),
                )
    ...
    fun `given watch providers link in region - when invoked - then link is passed through`() =
        runTest {
                ...
                FakeMoviesRepository(
                    ...
                    watchProviders =
                        WatchProvidersResponse(
                            ...
                            results =
                                mapOf(
                                    "DE" to WatchProviderRegion(link = "https://www.themoviedb.org/movie/1/watch"),
                                ),
                        ),
                )
            ...
            assertEquals("https://www.themoviedb.org/movie/1/watch", result.watchProvidersLink)
        }
}
